...
//--->number to word > start
	...
	var TENTHS_LESS_THAN_HUNDRED = [
	    'សូន្យ', 'ដប់', 'ម្ភៃ', 'សាមសិប', 'សែសិប', 'ហាសិប', 'ហុកសិប', 'ចិតសិប', 'ប៉ែតសិប', 'កៅសិប'
	];
	...
	function generateWords(number) {
	    ...
	    // We’re done
	    if (number === 0) {
	        return !words ? 'សូន្យ' : words.join(' ').replace(/,$/, '');
	    }
	    // First run
	    if (!words) {
	        words = [];
	    ...
	    if (number < 10) {
	        ...
	        word = generateWords(Math.floor(number / ONE_HUNDRED)) + 'រយ';

	    } else if (number < ONE_MILLION) {
	        remainder = number % ONE_THOUSAND;
	        word = generateWords(Math.floor(number / ONE_THOUSAND)) + 'ពាន់';

	    } else if (number < ONE_BILLION) {
	        remainder = number % ONE_MILLION;
	        word = generateWords(Math.floor(number / ONE_MILLION)) + ' មឺុន,';

	    } else if (number < ONE_TRILLION) {
	        remainder = number % ONE_BILLION;
	        word = generateWords(Math.floor(number / ONE_BILLION)) + ' លាន,';

	    } else if (number < ONE_QUADRILLION) {
	        remainder = number % ONE_TRILLION;
	        word = generateWords(Math.floor(number / ONE_TRILLION)) + ' កោដដិ,';
