        ...
        <div class="col-sm-6">
            <div class="caption font-dark" style="display: inline-block;">
                ...
                <a target="_blank" href="print_out.php?p_date_start=<?= (@$_POST['txt_date_start']?$_POST['txt_date_start']:date('Y-m-d')) ?>&p_date_end=<?= (@$_POST['txt_date_end']?$_POST['txt_date_end']:date('Y-m-d')) ?>" id="sample_editable_1_new" class="btn yellow">
                 ...
                 <a target="_blank" href="out_export_excell.php?p_date_start=<?= (@$_POST['txt_date_start'] ? $_POST['txt_date_start'] : date('Y-m-d')) ?>&p_date_end=<?= (@$_POST['txt_date_end'] ? $_POST['txt_date_end'] : date('Y-m-d')) ?>" id="sample_editable_1_new" class="btn blue">
        ...
        <table class="table table-striped table-bordered table-hover dataTable dtr-inline" id="sample_1" role="grid" aria-describedby="sample_1_info">
            <thead>
                <tr role="row" class="text-center">
                    ...
                    <th class="text-center">អ្នកទទួល</th>
                    ...
                    <th rowspan="2" class="text-center">ម៉ាស៊ីន/គ្រឿងចក្រ</th>
                    ...
                    while ($row = mysqli_fetch_object($get_data)) {
                        echo '<tr>';
                            ...
                            echo '<td class="text-center">';
                                ...
                                echo '<a href="delete_out.php?del_detail_id='.$row->std_id .'&parent_id='.$row->stsout_id.'" onclick="return confirm(\'Are you sure to delete this?\')" class="btn btn-xs btn-danger" title="delete"><i class="fa fa-trash"></i></a> ';
                            echo '</td>';
                        echo '</tr>';
                    }
                ?>
