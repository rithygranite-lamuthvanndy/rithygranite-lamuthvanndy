...
<?php 
    if(isset($_POST['btn_submit'])){
        $v_main_id = @$connect->real_escape_string($_POST['txt_main']);
        $v_name = @$connect->real_escape_string($_POST['txt_name']);
        $v_note = @$connect->real_escape_string($_POST['txt_note']);
        $query_add = "INSERT INTO tbl_hr_department_sub (
                dep_main_id,
                dep_name,
                dep_note) 
            VALUES(
                    ...
                    <div class="form-body">
                        <div class="row">
                            <div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
                                        <label>Deparment/ផ្នែក * : </label>
                                        <select name="txt_main" id="inputCbo_position" class="form-control myselect2" required="required">
                                            <option value="">*** Select and choose ***</option>
                                            <?php 
                                                $v_result=$connect->query("SELECT * FROM tbl_hr_department_main ORDER BY depm_id");
