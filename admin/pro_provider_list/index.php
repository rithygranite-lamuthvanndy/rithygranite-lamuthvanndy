...
<div class="portlet light bordered">
    <div class="row">
        <div class="col-xs-12">
            <h2><i class="fa fa-fw fa-map-marker"></i>Provider List</h2>
        </div>
    </div>
    <br>
    <div class="">
                ...
                <tbody>
                    <?php
                        ...
                        $get_data = $connect->query("SELECT 
                               *
                            FROM   tbl_pj_provider AS A  
                            ORDER BY dpro_id DESC");
                        while ($row = mysqli_fetch_object($get_data)) {
                            echo '<tr>';
                                echo '<td>'.(++$i).'</td>';
                                echo '<td>'.$row->dpro_name.'</td>';
                                echo '<td>'.$row->dpro_note.'</td>';
                                echo '<td class="text-center">';
                                    echo '<a href="edit.php?edit_id='.$row->dpro_id.'" class="btn btn-xs btn-warning" title="edit"><i class="fa fa-edit"></i></a> ';
