...
<?php 
    if(isset($_POST['btn_search'])){
        ...
        $get_data=$connect->query("SELECT 
                            A.*, (reiw_qty*reiw_price) as amount,B.reqw_id, date_format(B.reqw_date,'%d-%m-%Y') as datefor, B.reqw_number,C.uni_name FROM tbl_acc_reqw_item_ AS A
                            LEFT JOIN tbl_acc_request_wordsite AS B ON A.reiw_numberw=B.reqw_id
                            LEFT JOIN  tbl_acc_unit_list as C On A.reiw_unit=C.uni_id
            ...
            <table class="table table-striped table-bordered table-hover dataTable dtr-inline collapsed" id="sample_1" role="grid" aria-describedby="sample_1_info" style="width: 1180px;">
                    <thead>
                        <tr>
                            ...
                            <th style="vertical-align: middle; text-align: center;">
                                <label>កំណត់សកម្មភាណ <i class="fa fa-cog fa-spin"></i>
                                </label>
                            </th>
                        </tr>
                    </thead>
                    ...
                    <?php
                        ...
                        while ($row = mysqli_fetch_object($get_data)) {
                            echo '<tr>';
                                ...
                                echo '<td>'.$row->reiw_qty.'</td>';
                                echo '<td>'.$row->uni_name.'</td>';
                                echo '<td class="text-right">$ '.number_format($row->reiw_price,  2) .'</td>';
                                echo '<td class="text-right">$ '.number_format($row->amount,  2) .'</td>';
                                echo '<td>'.$row->reiw_limit.'</td>';
                                echo '<td>'.$row->reiw_date_li.'</td>';
                                echo '<td>'.$row->reiw_note.'</td>';
                                echo '<td class="text-center">';
                                echo '<a target="_blank" href="print.php?print_id=' . $row->reqw_id . '" class="btn btn-xs btn-info" title="edit"><i class="fa fa-print"></i></a> ';
