<?php 
    ...
    include_once 'global_funtion.php';
?>
<?php 
    ...
    <div class="main_border">
        <?php 
            if($_SESSION['status']==1){
                echo '<h3>សម្ភារប្រើប្រាស់នៅក្នុងរោងចក្រ '.date('d/m/Y',strtotime($v_date_start)).' - '.date('d/m/Y',strtotime($v_date_end)).'</h3>';
            }
            else if($_SESSION['status']==2){
                echo '<h3>សម្ភារប្រើប្រាស់នៅក្នុងរោងចក្រ</h3>';
            }
         ?>
        <h4>VẬT TƯ SỬ DỤNG TRONG NHÀ MÁY - THÁNG</h4>
        <br> 
        <!-- Table Decription -->
        <table>
            <tbody>
                ...
                <?php
                    ...
                    $v_total_price_beg=0;
                    $v_total_price_in=0;$v_total_qty_in=0;
                    $v_total_price_out=0;$v_total_qty_out=0;
                    $v_total_price_adjust=0;$v_total_qty_adjust=0;
                    $v_total_price_bal=0;$v_total_qty_bal=0;
                    $v_arr_summary=[];
