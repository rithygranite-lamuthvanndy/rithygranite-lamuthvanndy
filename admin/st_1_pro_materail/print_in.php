    ...
    <div class="main_border">
        <?php 
            if($_SESSION['status']==1){
                echo '<h3>តារាងបញ្ចូលស្ដុកសំភារៈផ្កត់ផ្គង់ក្នុងរណ្ដៅប្រចាំថ្ងៃ</h3>';
            }
            else if($_SESSION['status']==2){
                echo '<h3>តារាងបញ្ចូលស្ដុកសំភារៈផ្កត់ផ្គង់រោងចក្រប្រចាំថ្ងៃ</h3>';
            }
         ?>
        <h4>Bảng Nhập Kho Vật tư Nhà Máy Hàng Ngày</h4>
        <br> 
        <!-- Table Decription -->
        <table>
            <tbody>
                ...
                <?php
                    ...
                    while ($row = mysqli_fetch_object($get_data)) {
                        echo '<tr>';
                            ...
                            echo '<td>'.date('d/M/Y',strtotime($row->stsin_date_in)).'</td>';
                            ...
                            echo '<td class="myqty">'.$row->in_qty.'</td>';
                            echo '<td>'.$row->stun_name.'</td>';
                            echo '<td class="myprice_vn">'.$row->in_price_vn.'</td>';
                            echo '<td class="myprice">'.$row->in_price_dollar.'</td>';
                            echo '<td class="myprice">'.number_format($row->total_amo,2).'</td>';
                        echo '</tr>';
                    }
                ?>
            </tbody>
