...
<?php 
    ...
    // add 
        ...
        $query_add = "INSERT INTO tbl_sup_bill_alert (
                supba_date_alert,
                supba_invoice_no,
                supba_note
    ...
    if(@$_GET['del_id'] != ""){
        $del_id = @$_GET['del_id'];
        $connect->query("DELETE FROM tbl_sup_bill_alert WHERE supba_id='$del_id'");
        header('location: alert.php?alert_id='.$cus_inv_id);
    }
        ...
        <div id="sample_1_wrapper" class="dataTables_wrapper">
            <table class="table table-striped table-bordered table-hover dataTable dtr-inline collapsed" id="sample_1" role="grid" aria-describedby="sample_1_info">
                ...
                <tbody>
                    <?php
                        $i = 0;
                        $get_data = $connect->query("SELECT  * FROM tbl_sup_bill_alert AS A 
                            LEFT JOIN tbl_sup_bill AS B ON B.supb_id=A.supba_invoice_no WHERE supba_invoice_no='$cus_inv_id' ORDER BY supba_date_alert DESC");
                        while ($row = mysqli_fetch_object($get_data)) {
                            echo '<tr>';
                                echo '<td>'.(++$i).'</td>';
                                echo '<td>'.$row->supba_date_alert.'</td>';
                                echo '<td>'.$row->supb_invoice_no.'</td>';
                                echo '<td>'.$row->supba_note.'</td>';
                                echo '<td class="text-center">';
                                    echo '<a href="alert.php?del_id='.$row->supba_id.'&alert_id='.$cus_inv_id.'" onclick="return confirm(\'Are you sure to delete this?\')" class="btn btn-xs btn-danger" title="delete"><i class="fa fa-trash"></i></a> ';
