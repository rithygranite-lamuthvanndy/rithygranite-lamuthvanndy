...
<?php 
    ...
    $v_grade_type_id=@$_GET['grade_type_id'];

    if($v_grade_type_id){
        $obj_select=$connect->query("SELECT COUNT(*) AS count,
                (SELECT name FROM tbl_inv_grade_type_list WHERE id='$v_grade_type_id') AS grade_type_name
            FROM tbl_inv_1_stock_block_stone_detail 
            WHERE grade_type_id='$v_grade_type_id'");
        $row_count=mysqli_fetch_object($obj_select);
        $new_block_code=$row_count->grade_type_name.'-'.sprintf("%'.04d",$row_count->count+1);
