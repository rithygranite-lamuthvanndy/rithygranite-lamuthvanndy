...
if (isset($_POST['btn_submit'])) {
    ...
    $v_po_saleorder = @$_POST['cbo_sale_order'];
                            ...
                            <div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
                                ...
                                <div class="form-group ">
                                    <label>Template : </label>
                                        <select name="cbo_sale_order" id="input" class="form-control myselect2" required="required">
                                            <option value="">*** Select and choose ***</option>
                                            <?php 
                                                $v_result=$connect->query("SELECT * FROM tbl_sale_order ORDER BY tso_id");
                                                while ($row_select=mysqli_fetch_object($v_result)) {
                                                echo '<option data_code="'.$row_select->tso_code.'" value="'.$row_select->tso_id.'">'.$row_select->tso_code.' || '.$row_select->tso_name_kh.'|| '.$row_select->tso_name_en.'</option>';
...
<script>
    $(document).ready(function(){
        $('select[name=cbo_sale_order]').change(function () {
            ...
            $.ajax({type:'POST',url:"ajx_get_po.php",data:'data='+myString,success: function(result){
        ...
        $('input[name=txt_date]').datepicker().on('changeDate', function (ev) {
            ...
            var vou_id=$('select[name=cbo_sale_order]').val();
            var vou_id=$('select[name=cbo_sale_order]').val();
            code_vouch=$('select[name=cbo_sale_order]').find('option:selected').attr('data_code');
            var myObject={
                tso_id:null,
                vou_code:code_vouch,
                v_date:v_date,
            ...
            var myString =JSON.stringify(myObject);
            $.ajax({url:"ajx_get_po.php",type:'POST',data:'data='+myString,success: function(result){
                var myObj=JSON.parse(result);
                $('input[name=txt_number]').val(myObj['vou_no']);
                document.getElementById("pono_code").innerHTML = myObj['vou_no'];
            }});
