...
<?php
    if(@$_GET['v_id']){
        $v_id=@$_GET['v_id'];
        $get_data=$connect->query("SELECT A.*,acc_id,SUM(debit) AS total_debit,SUM(credit) AS total_credit,accca_number,accca_account_name 
            FROM tbl_acc_add_tran_dr_cr_detail AS A 
            LEFT JOIN tbl_acc_chart_account AS B ON A.acc_id=B.accca_id
            WHERE A.main_id='$v_id' 
            GROUP BY B.accca_id");
                    ...
                    <?php
                        ...
                        while ($row = mysqli_fetch_object($get_data)) {      
                            echo '<tr>';
                                ...
                                echo '<td class="text-left">$ '.number_format($row->unit_price,2).'</td>';
                                echo '<td class="text-left">$ '.number_format($row->qty*$row->unit_price,2).'</td>';
                                echo '<td class="text-left">$ '.number_format($row->debit_old,2).'</td>';
                                ...
                                echo '<td class="text-left">$ '.number_format($row->total_debit,2).'</td>';
                                echo '<td class="text-left">$ '.number_format($row->total_credit,2).'</td>';
                            echo '</tr>';
                            $tot_debit+=$row->total_debit;
                            $tot_credit+=$row->total_credit;
                        }
                    ?>
                </tbody>
                <tfoot>
