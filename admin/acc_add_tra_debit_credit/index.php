...
if(isset($_POST['btn_search'])){
    ...
    $get_data = $connect->query("SELECT  A.id AS id_main,A.p_appr,ref_id,A.date_record,A.status_type,
    ...
    $get_data = $connect->query("SELECT A.id AS id_main,A.p_appr,ref_id,A.date_record,A.status_type,
        ...
        FROM tbl_acc_add_tran_dr_cr AS A 
        LEFT JOIN tbl_acc_add_tran_dr_cr_detail AS B ON A.id=B.main_id
        WHERE DATE_FORMAT(A.date_audit,'%Y-%m')='$v_current_year_month'
        GROUP BY A.id"); 
}
if(isset($_POST['txt_new_row'])){
    $_SESSION['new_row_acc_add_tra_debit_credit']=$_POST['txt_new_row'];
}
...
<div class="portlet light bordered">
    ...
    <ul class="nav nav-tabs" role="tablist">
        <li role="presentation">
            <a href="../acc_add_tra_amount/">Transation Amount</a>
        </li>
        <li role="presentation" class="active">
            <a href="../acc_add_tra_debit_credit/">Trasation Debit Credit</a>
        </li>
    </ul>
    <br>
    <div class="">
                    ...
                    <li >
                        <a class="tool-action">
                            <i class="icon-printer"></i>
                                Custom Row Page
                                <input name="txt_new_row" form="form" value="<?= (@$_SESSION['new_row_acc_add_tra_debit_credit'])?(@$_SESSION['new_row_acc_add_tra_debit_credit']):0 ?>" class="form-control" type="number" onsubmit="this.form,submit()" value="0">
                        </a>
                    </li>
                </ul>
            </div> 
                        ...
                        while ($row = mysqli_fetch_object($get_data)) {
                            echo '<tr>';
                                ...
                                if($row->status_type==1){
                                    ...
                                    $sql=$connect->query("SELECT entry_no FROM tbl_acc_rec_adjustment WHERE id='$row->ref_id'");
                                    $row_tmp=mysqli_fetch_object($sql);
                                    echo '<td>'.$row_tmp->entry_no.'</td>';
                                }
                                else if($row->status_type==2){
                                    echo '<td>IV Sale Revenue</td>';
                                    $sql=$connect->query("SELECT entry_no FROM tbl_acc_rec_stock_inventory WHERE id='$row->ref_id'");
                                    $row_tmp=mysqli_fetch_object($sql);
                                    echo '<td>'.$row_tmp->entry_no.'</td>';
                                }
                                else if($row->status_type==3){
                                    echo '<td>Transfer Funds</td>';
                                    $sql=$connect->query("SELECT tran_ref_no AS entry_no FROM tbl_acc_add_transfer_fund WHERE id='$row->ref_id'");
                                    $row_tmp=mysqli_fetch_object($sql);
                                    echo '<td>'.$row_tmp->entry_no.'</td>';
                                }
                                ...
                                echo '<a target="_blank" href="../acc_add_tra_amount/print.php?print_id='.$row->id_main.'&status=tra_debit_credit" class="btn btn-xs btn-danger" title="Print"><i class="fa fa-print"></i></a> ';
...
<script type="text/javascript">
    ...
    function change_view (args) {
        let v_check=$(args).prop('checked');
        let v_man_id=$(args).parents('tr').find('td:nth-child(1)').find('input').attr('data-parent-id');
        let arr=new Array(v_check,v_man_id);
        // alert(arr);
    ...
    function change_views (args) {
        ...
        if(check){
            $('td:nth-child(1)').find('label>input').prop("checked", !$(this).prop("checked"));
            $('td:nth-child(1)').find('label>input').each(function () {
                var v_view=$(this).prop('checked');
                var v_man_id = $(this).parents('tr').find('td:nth-child(1)').find('input').attr('data-parent-id');
                // arr={v_view,v_man_id};
                let arr=new Array(v_view,v_man_id);
                $.ajax({url: "ajax_update_appro.php",
                    type: "POST",
                    data: "myData="+arr,
                    async: false,
            ...
            $('td:nth-child(1)').find('label>input').removeAttr('checked');
            $('td:nth-child(1)').find('label>input').each(function () {
                var v_view=$(this).prop('checked');
                var v_man_id = $(this).parents('tr').find('td:nth-child(1)').find('input').attr('data-parent-id');
                let arr=new Array(v_view,v_man_id);
