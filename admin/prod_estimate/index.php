...
<div class="portlet light bordered">
    <div class="row">
        <div class="col-xs-12">
            <h2 style="font-family: 'Khmer OS';"><i class="fa fa-fw fa-map-marker"></i> តំរងឯកសារតម្លៃទំនិញ</h2>
        </div>
    </div>
    <br>
    <div class="">
                ...
                <tbody>
                    <?php
                        ...
                        $get_data = $connect->query("SELECT 
                               *
                            FROM tbl_estimate AS A 
                            ORDER BY te_code ASC");
                        while ($row = mysqli_fetch_object($get_data)) {
                            echo '<tr>';
                                echo '<td>'.(++$i).'</td>';
                                echo '<td>'.$row->te_code.'</td>';
                                echo '<td>'.$row->te_name_kh.'</td>';
                                echo '<td>'.$row->te_name_en.'</td>';
                                echo '<td>'.$row->te_name_vn.'</td>';
                                echo '<td>'.$row->te_note.'</td>';
                                echo '<td class="text-center">';
                                    echo '<a href="edit.php?edit_id='.$row->te_id.'&view='.@$_GET['view'].'" class="btn btn-xs btn-warning" title="edit"><i class="fa fa-edit"></i></a> ';
                                   echo '<a onclick="deleteRecord('.$row->te_id.')" class="btn btn-xs btn-danger" title="delete"><i class="fa fa-trash"></i></a> ';
