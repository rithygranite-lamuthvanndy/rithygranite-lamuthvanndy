<?php
    ...
    function button_add(){
    	...
    	if($row_action_permission->p_add){
    		return '<a href="add.php" id="sample_editable_1_new" class="btn green"> Add New
    ...
    function button_delete($id,$img){
    	global $row_action_permission;
    	if($row_action_permission->p_delete){
        	return '<a href="delete.php?del_id='.$id.'&del_img='.trim($img).'" onclick="return confirm(\'Are u sure to  delete this user?\')" class="btn btn-xs btn-danger" title="delete"><i class="fa fa-trash"></i></a> ';
        }else{
