...
<div class="portlet light bordered">
    <div class="row">
        <div class="col-xs-12">
            <?= @$sms; ?>
            <h2>
                <i class="fa fa-plus-circle fa-fw"></i>បញ្ចូលទិន្នន័យបៀវត្សន៍
            </h2>
                        ...
                        <div class="row">
                            <div class="col-xs-6 col-sm-4 col-md-4 col-lg-2">
                                <div class="form-group">
                                    <label>បៀវត្សន៍ប្រចាំ/ Salary Monthly *:</label>
                                    <input type="text" data-provide="datepicker" data-date-format="yyyy-M" class="form-control" required name="txt_date_record" value="<?= date('Y-m') ?>">
                                </div>
                            </div>
                            <div class="col-xs-6 col-sm-4 col-md-4 col-lg-2">
                                <div class="form-group">
                                    <label>តំបន់ / Location *:</label>
                    ...
                    <table id="myTable" class="table table-bordered table-responsive">
                        <thead>
                            <tr>
                                ...
                                <th class="text-center">Day Work</th>
                                <th class="text-center">Increase<br>Decrease</th>
                                <th class="text-center">Allowance<br>Deduct</th>
                                <th class="text-center">Borrow<br>Last MT</th>
                                <th class="text-center">Borrow<br>On MT</th>
                                <th class="text-center">Reduce</th>
                                <th class="text-center">Balance<br>Loan</th>
                        ...
                        <tbody>
                            ...
                            <tr class="my_form_base" style="background: red; display: none;">
                                <td>
                                    <input type="number" name="txt_mater_kub[]" class="form-control" readonly="" value="1">
                                </td>
                                <td>
                                                <select class="form-control" onchange="changenameen(this);" name="cbo_acc_no[]">
                                                    <option value="">=== Please Choose and Select ===</option>
                                                    <?php 
                                                        $v_select = $connect->query("SELECT empl_id,empl_emloyee_en FROM tbl_hr_employee_list ORDER BY empl_emloyee_en DESC");
                                                        while ($row_data = mysqli_fetch_object($v_select)) {
                                                            echo '<option value="'.$row_data->empl_id.'">'.$row_data->empl_emloyee_en.'</option>';
                                                        }
                                                     ?>
                                                </select>
                                </td>
                                <td>
                                                <select class="form-control" onchange="changenamekh(this);">
                                                    <option value="">=== Please Choose and Select ===</option>
                                                    <?php 
                                                        $v_select = $connect->query("SELECT empl_id,empl_emloyee_kh FROM tbl_hr_employee_list ORDER BY empl_emloyee_kh DESC");
                                                        while ($row_data = mysqli_fetch_object($v_select)) {
                                                            echo '<option value="'.$row_data->empl_id.'">'.$row_data->empl_emloyee_kh.'</option>';
                                                        }
                                                     ?>
                                                </select>
                                </td>
                                <td>
                                    <input type="number" onchange="changenameen(this);" name="txt_salary[]" class="form-control">
                                </td>
                                <td>
                                    <input type="number" name="txt_block_code[]" class="form-control" value="30">
                                </td>
                                <td>
                                    <input type="number" onchange="increase_decrease(this);" onkeyup="increase_decrease(this);" step="0.01" name="txt_lenght[]" class="form-control"value="0.01">
                                </td>
                                <td>
                                ...
                                <td>
                                    <input type="number" step="0.01" name="txt_mater_kub[]" class="form-control" readonly="" value="">
                                </td>
...
<script>
    ...
    function increase_decrease (args) {
        let v_salary=$(args).parents('tbody >tr').find('td:nth-child(4) >input').val();
        let v_dwork=$(args).parents('tbody >tr').find('td:nth-child(5) >input').val();
        let v_inde=$(args).val();
        let v_adeduct=$(args).parents('tbody >tr').find('td:nth-child(7) >input').val();
        let v_reduce=$(args).parents('tbody >tr').find('td:nth-child(10) >input').val();
        let v_npay=sum_salary(v_salary,v_dwork,v_inde,v_adeduct,v_reduce);
        $(args).parents('tbody >tr').find('td:nth-child(12) >input').val(v_npay);
        totalAmount();
    }
    ...
    function changenameen (args) {
        var v_acc_id=$(args).val();
        $.ajax({url: 'ajx_get_content_select_name.php?v_acc_id='+v_acc_id+"&status=id",success:function (result) {
            $(args).parents('tbody >tr').find('td:nth-child(3) >select').html(result);
        }});
        $.ajax({url: 'ajx_get_content_select_salary.php?v_acc_id='+v_acc_id+"&status=id",success:function (result) {
            $(args).parents('tbody >tr').find('td:nth-child(4) >select').html(result);
        }});
    }
    function changenamekh (args) {
        var v_acc_id=$(args).val();
        $.ajax({url: 'ajx_get_content_select_name.php?v_acc_id='+v_acc_id+"&status=name",success:function (result) {
            $(args).parents('tbody >tr').find('td:nth-child(2) >select').html(result);
        }});
        $.ajax({url: 'ajx_get_content_select_salary.php?v_acc_id='+v_acc_id+"&status=name",success:function (result) {
            $(args).parents('tbody >tr').find('td:nth-child(4) >select').html(result);
        }});
    }
</script>
