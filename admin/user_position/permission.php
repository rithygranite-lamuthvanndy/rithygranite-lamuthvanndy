<?php
// echo $left_menu;
    $get_action_permission = $connect->query("SELECT * FROM tbl_permission WHERE p_position=".$_SESSION['user']->user_position." AND p_module='4'");
    ...
    function button_edit_permission($id){
        global $row_action_permission;
        if($row_action_permission->p_edit){
            return '<a href="set_permission.php?parent_id='.$id.'" class="btn btn-xs btn-primary">Permission*</a>';
        }else{
            return '<a href="#" class="btn btn-xs btn-primary disabled">Permission*</a>';
        }
    }
