...
<div class="portlet light bordered">
    <div class="row">
        <div class="col-xs-12">
            <h2><i class="fa fa-fw fa-map-marker"></i> Purchase Check (Office)</h2>
        </div>
    </div>
    <br>
    <br>
            ...
            <table class="table table-striped table-bordered table-hover dataTable dtr-inline collapsed" id="sample_1" role="grid" aria-describedby="sample_1_info" style="width: 1180px;">
                <thead>
                    <tr role="row" class="text-center">
                        ...
                        <th>Check By </th>
                        <th>Approved By </th>
                        <th>Purchase Check Office By</th>
                        <th>Purchase Check Office Date</th>
                        <th>Purchase Check Office Comment</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                        $get_data=$connect->query("SELECT * FROM tbl_acc_request_form AS A
                            ...
                            WHERE (A.st_check_by AND A.adm_check_by 
                            AND A.sub_man_app_by AND A.man_app_by  
                            AND A.admin_check_off_by) <>0
                        ...
                        while ($row = mysqli_fetch_object($get_data)) {
                            echo '<tr>';
                                ...
                                echo check_list($row->req_id,$row->pur_check_id);
                                echo '</td>';
                                ...
                                $sql=$connect->query("SELECT * 
                                    ...
                                    LEFT JOIN tbl_acc_approved_name_list AS B ON A.ceo_app_by=B.apn_id WHERE req_id='$row->req_id'");
                                $row_1=mysqli_fetch_object($sql);
                                echo '<td>'.$row_1->apn_name.'</td>';
                                echo '<td>';
                                    if($row->ceo_app_date=='0000-00-00')
                                        echo '';
                                    else
                                        echo ($row->ceo_app_date);
                                echo '</td>';
                                echo '<td>'.$row->ceo_app_note.'</td>';
                            echo '</tr>';
                        }
                    ?>
                </tbody>
