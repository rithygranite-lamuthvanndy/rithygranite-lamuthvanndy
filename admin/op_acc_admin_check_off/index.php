...
<div class="portlet light bordered">
    <div class="row">
        <div class="col-xs-12">
            <h2><i class="fa fa-fw fa-map-marker"></i> Admin Office Check List</h2>
        </div>
    </div>
    <br>
    <br>
            ...
            <table class="table table-striped table-bordered table-hover dataTable dtr-inline collapsed" id="sample_1" role="grid" aria-describedby="sample_1_info" style="width: 1180px;">
                <thead>
                    <tr role="row" class="text-center">
                        ...
                        <th>Check By </th>
                        <th>Approved By </th>
                        <th>Admin Check (Office) By</th>
                        <th>Admin Check (Office) Date</th>
                        <th>Admin Check (Office) Comment</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                        $get_data=$connect->query("SELECT * FROM tbl_acc_request_form AS A
                            ...
                            WHERE (A.st_check_by AND A.adm_check_by AND A.sub_man_app_by) <>0
                        ...
                        while ($row = mysqli_fetch_object($get_data)) {
                            echo '<tr>';
                                ...
                                echo check_list($row->req_id,$row->admin_check_off_by);
                                echo '</td>';
                                ...
                                $sql=$connect->query("SELECT * 
                                    ...
                                    LEFT JOIN tbl_acc_check_name_list AS B ON A.admin_check_off_by=B.chn_id WHERE req_id='$row->req_id'");
                                $row_1=mysqli_fetch_object($sql);
                                echo '<td>'.$row_1->chn_name.'</td>';
                                echo '<td>';
                                    if($row->admin_check_off_date=='0000-00-00')
                                        echo '';
                                    else
                                        echo ($row->admin_check_off_date);
                                echo '</td>';
                                echo '<td>'.$row->admin_check_off_note.'</td>';
                            echo '</tr>';
                        }
                    ?>
                </tbody>
