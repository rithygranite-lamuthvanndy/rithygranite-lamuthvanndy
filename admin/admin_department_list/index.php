            ...
            <table class="table table-striped table-bordered table-hover dataTable dtr-inline collapsed" id="sample_1" role="grid" aria-describedby="sample_1_info" style="width: 1180px;">
                <thead>
                    <tr role="row" class="text-center">
                        ...
                        <th class="text-center">Department Note</th>
                ...
                <tbody>
                    <?php
                        $i = 0;
                        $get_data = $connect->query("SELECT * FROM tbl_admin_department_list ORDER BY dep_id DESC");
                        while ($row = mysqli_fetch_object($get_data)) {
                            echo '<tr>';
                                echo '<td class="text-center">'.(++$i).'</td>';
                                echo '<td class="text-center">'.$row->dep_name.'</td>';
                                echo '<td class="text-center">'.$row->dep_note.'</td>';
