...
<?php 
	...
	if(@$_GET['del_detail_id']){
		...
		$v_sql_check="SELECT stsout_id FROM tbl_st_stock_out_detail WHERE stsout_id='$v_parent_id'";
		if(!mysqli_num_rows($connect->query($v_sql_check))){
			$connect->query("DELETE FROM tbl_st_stock_out WHERE stsout_id='$v_parent_id'");
		}
