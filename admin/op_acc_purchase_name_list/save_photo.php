...
<?php
	...
	// Upload directory
	$target_dir = "../../img/sign_name/";
	$v_sql="SELECT img FROM tbl_acc_purchase_name_list WHERE purn_id='$v_parent_id'";
	...
	if(move_uploaded_file($v_image["tmp_name"], $target_dir.$new_name)) {
		if($v_parent_id)
	    	$v_sql_update=$connect->query("UPDATE tbl_acc_purchase_name_list 
	                                    SET img='$new_name'
	                                    WHERE purn_id='$v_parent_id'
	                                    ");
	   	else{
