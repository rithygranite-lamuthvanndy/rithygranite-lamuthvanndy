...
<div class="portlet light bordered">
    <div class="row">
        <div class="col-xs-12">
            <h2><i class="fa fa-fw fa-map-marker"></i> Cash Balance: Search by category</h2>
        </div>
    </div>
    <br>
    <div class="row">
        <form action="<?= $_SERVER['PHP_SELF'] ?>" method="post">
            ...
            <div class="col-sm-2">
                <div class="" >
                    <select name="txt_find" class="form-control">
                        <option value="">Choose: Category</option>
                        <?php 
                            $v_select = $connect->query("SELECT * FROM tbl_cf_category_list ORDER BY cfcl_name ASC");
                            while($row_cate = mysqli_fetch_object($v_select)){
                                if($row_cate->cfcl_id == @$_POST['txt_find']){
                                    echo '<option SELECTED value="'.$row_cate->cfcl_id.'">'.$row_cate->cfcl_name.'</option>';

                                }else{
                                    echo '<option value="'.$row_cate->cfcl_id.'">'.$row_cate->cfcl_name.'</option>';
                    ...
                    <?php
                        ...
                        if(isset($_POST['btn_search'])){
                            ...
                            if(@$_POST['txt_find']!=""){
                                ...
                                WHERE cfcr_date_record BETWEEN '$v_date_s' AND '$v_date_e' AND A.cfcr_category='$v_find'
