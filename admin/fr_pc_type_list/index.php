        ...
        <div class="col-xs-3">
            <li class="dropdown dropdown-dark" width="400px">
                <a href="javascript:;" class="dropdown-toggle" data-toggle="dropdown" data-hover="dropdown" data-close-others="true">
                    <h2><i class="fa fa-fw fa-map-marker" style="color: lightgreen;"></i><span class="username username-hide-on-mobile"> FR-PC Type</span></h2>
                </a>
...
<?php 
    if(isset($_POST['btn_submit'])){
        ...
        $query_add = "INSERT INTO tbl_fr_pc_type_list (
                fpt_code,
                fpt_name,
                fpt_note    
                ) 
...
<div class="portlet light bordered">
    ...
    <div class="portlet-body">
      ...
      <div class="row" id="myDIV">
        ...
        <?php
                        $get_data1 = $connect->query("SELECT *, (select sum(frpc_amount) from tbl_fr_pc_expense WHERE frpc_type=fpt_id) as totol_fpt FROM tbl_fr_pc_type_list ORDER BY fpt_name ASC");
        ...
        echo '<p><h4>'.$row1->fpt_code.' == '.$row1->fpt_name.'</h4></p>';
        echo '<p><h5>'.$row1->fpt_note.'</h5></p>';
        echo '</div>
            <div class="icon">
                ...
                <tbody>
                    <?php
                        ...
                        $get_data = $connect->query("SELECT 
                               *
                            FROM   tbl_fr_pc_type_list
                            ORDER BY fpt_id DESC");
                        while ($row = mysqli_fetch_object($get_data)) {
                            echo '<tr>';
                                echo '<td>'.(++$i).'</td>';
                                echo '<td>'.$row->fpt_code.'</td>';
                                echo '<td>'.$row->fpt_name.'</td>';
                                echo '<td>'.$row->fpt_note.'</td>';
                                echo '<td class="text-center">';
                                    echo '<a href="edit.php?edit_id='.$row->fpt_id.'" class="btn btn-xs btn-warning" title="edit"><i class="fa fa-edit"></i></a> ';
