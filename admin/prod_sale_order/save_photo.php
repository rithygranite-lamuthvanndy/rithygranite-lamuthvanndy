...
<?php
	...
	$v_sql="SELECT tit_pictures FROM tbl_items_type_list WHERE tit_id='$v_parent_id'";
	...
	if(move_uploaded_file($v_image["tmp_name"], $target_dir.$new_name)) {
		if($v_parent_id)
	    	$v_sql_update=$connect->query("UPDATE tbl_items_type_list 
	                                    SET tit_pictures='$new_name'
	                                    WHERE tit_id='$v_parent_id'
	                                    ");
	   	else{
