                ...
                <tbody>
                    <?php
                        ...
                        $get_data = $connect->query("SELECT 
                               *
                            FROM tbl_cf_monthyear_list AS A
                            LEFT JOIN tbl_user AS U ON U.user_id=A.user_id
                            ORDER BY cfmy_id DESC");
                        while ($row = mysqli_fetch_object($get_data)) {
                            echo '<tr>';
                                echo '<td>'.(++$i).'</td>';
                                echo '<td>'.$row->cfmy_name.'</td>';
                                echo '<td>'.$row->cfmy_note.'</td>';
                                echo '<td>'.$row->user_name.'</td>';
                                echo '<td>'.$row->date_audit.'</td>';
                                echo '<td class="text-center">';
                                    echo '<a href="edit.php?edit_id='.$row->cfmy_id.'" class="btn btn-xs btn-warning" title="edit"><i class="fa fa-edit"></i></a> ';
