 ...
 function belen_price($v_date_start,$v_date_end,$pro_name_id) {
    ...
    // start stock_early
       $v_sql_eanly = "SELECT A.*,B.stca_name,B.stca_code,C.stun_name,B.stca_id,
                ...
                WHERE A.stpron_material_type='3' AND A.stpron_id='$pro_name_id' 
                HAVING 
    ...
    <div class="row">
        ...
        <div class="col-xs-5">
             <div class="caption font-dark" style="display: inline-block;">
                        ...
                        <a target="_blank" href="print_truck_repare.php?p_date_start=<?= (@$_POST['txt_month_start'] ? $_POST['txt_month_start'] : date('Y-m-d')) ?>&p_date_end=<?= (@$_POST['txt_month_end'] ? $_POST['txt_month_end'] : date('Y-m-d')) ?>&id=<?php echo @$_POST['txt_truck_name'] ?>" id="sample_editable_1_new" class="btn yellow">
  ...
  GROUP BY tbl_group_truck.id
    ...
    while ($rows = mysqli_fetch_object($get_data_truck)) {
      ...
      $v_sql_truck_cate= "SELECT 
                                    ...
                                    B.out_qty,
                        D.stun_name,B.stsout_id,B.locaton_id,C.stpron_code,A.stsout_note,
                        F.supsi_name,C.stpron_name_kh,C.stpron_id as pro_name_id,
    ...
    $status_kemun1=0;
     ...
     while ($row= mysqli_fetch_object($get_truck_belen)) {
       ...
       $v_sql_eanly = "SELECT A.*,B.stca_name,B.stca_code,C.stun_name,B.stca_id,
                ...
                LEFT JOIN tbl_st_category_list AS B ON A.stpron_category=B.stca_id
                LEFT JOIN tbl_st_unit_list AS C ON A.stpron_unit=C.stun_id
                WHERE A.stpron_material_type='3' AND A.stpron_id='$p_name_id' 
                HAVING 
    ...
    $a_pro_id_early_section="";
     ...
     while ($row= mysqli_fetch_object($get_truck_belen)) {
       ...
       $v_sql_eanly = "SELECT A.*,B.stca_name,B.stca_code,C.stun_name,B.stca_id,
                ...
                LEFT JOIN tbl_st_category_list AS B ON A.stpron_category=B.stca_id
                LEFT JOIN tbl_st_unit_list AS C ON A.stpron_unit=C.stun_id
                WHERE A.stpron_material_type='3' AND A.stpron_id='$p_name_id_early_section' 
                HAVING 
...
//                     }
  ...
  echo '<input type="hidden" class="section_data" value=" '.$total_stock_in_section3.' " ';
                 ...
                 ?>
                       ...
                       <tr class="bg_col_tr">
                        <th colspan="2" style="text-align: center;font-size: 14px;">TOTAL/CONG</th>
                        <th><?php echo number_format($location_early_section,2); ?></th>
                        ...
                        <th><?php echo number_format($total_by_cate_location,2); ?></th>
