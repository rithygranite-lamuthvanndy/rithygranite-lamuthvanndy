...
<div class="portlet light bordered">
    <div class="row">
        <div class="col-xs-12">
            <h2><i class="fa fa-fw fa-map-marker"></i> Project Processing</h2>
        </div>
    </div>
    <br>
    <div class="">
                ...
                <tbody>
                    <?php
                        ...
                        $get_data = $connect->query("SELECT 
                               *,A.user_id AS id, A.date_audit AS audit
                            ...
                            ORDER BY ppro_id DESC");
                        while ($row = mysqli_fetch_object($get_data)) {
                            echo '<tr>';
                                echo '<td>'.(++$i).'</td>';
                                echo '<td>'.$row->ppro_date_record.'</td>';
                                echo '<td>'.$row->pti_project_title.'</td>';
                                echo '<td>'.$row->pini_project_sub.'</td>';
                                echo '<td>'.$row->dper_name.'    %</td>';
                                echo '<td>'.$row->ppro_note.'</td>';
                                echo '<td class="text-center">';
                                    echo '<a href="edit.php?edit_id='.$row->ppro_id.'" class="btn btn-xs btn-warning" title="edit"><i class="fa fa-edit"></i></a> ';
                                    // echo '<a href="delete.php?del_id='.$row->ppro_id.'" onclick="return confirm(\'Are you sure to delete this?\')" class="btn btn-xs btn-danger" title="delete"><i class="fa fa-trash"></i></a> ';
