<?php
    ...
    function attach_file(){
        ...
        if($row_action_permission->p_add){
            return '<a data-toggle="modal" href="#modal_add" id="sample_editable_1_new" class="btn green">
    ...
    function button_delete_file($id,$file_name,$sent_id){
        global $row_action_permission;
        if($row_action_permission->p_delete){
            return '<a href="?del_id='.$id.'&old_file='.$file_name.'&sent_id='.$sent_id.'" class="btn btn-xs btn-danger"><i class="fa fa-trash"></i></a> ';
        }else{
            return '<a class="btn btn-xs btn-danger disabled" title="delete"><i class="fa fa-trash"></i></a> ';
        }
    }
