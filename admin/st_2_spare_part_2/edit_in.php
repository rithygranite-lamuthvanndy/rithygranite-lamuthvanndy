                    ...
                    <div class="form-body">
                        ...
                        <div class="row" style="overflow-x: scroll;">
                            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                                <table id="myTable" class="table table-bordered" style="width: 150%; overflow-x: scroll;">
                                    <thead>
                                        <tr>
                                            <th class="text-center" style="width: 90%;">កូដ</th>
                                            ...
                                            <th class="text-center" style="width: 20%;">គ្រឿងចក្រឬម៉ាស៊ីន</th>
                                            <th class="text-center" style="width: 90%;">តំបន់</th>
                                        ...
                                        <tr>
                                            <th class="text-center" style="width: 90%;">Mã</th>
                                            <th class="text-center" style="width: 7%;">số lượng</th>
                                            <th class="text-center" style="width: 90%;">Đơn vị</th>
                                            <th class="text-center" style="width: 15%;">Tổng công</th>
                                            <th class="text-center" style="width: 3%;">Đơn vị</th>
                                            <th class="text-center" style="width: 3%;">Tổng công</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php 
                                            $v_sql_child="SELECT *,
                                                        ...
                                                        INNER JOIN tbl_st_stock_in  AS B ON A.stsin_id=B.stsin_id
                                                        INNER JOIN tbl_st_product_name AS C ON A.pro_id=C.stpron_id

                                                         INNER JOIN tbl_st_stock_out_detail AS D ON D.pro_id=C.stpron_id

                                                        INNER JOIN tbl_st_track_machine_list AS E ON E.id=D.track_mac_id
                                                        WHERE A.stsin_id='$edit_id' AND D.in_id='$edit_id'
                                                        ";
                                          ...
                                          // echo $v_sql_child;
                                            ...
                                            while ($row_child=mysqli_fetch_object($v_result_child)) {
                                                ...
                                                echo '<tr>';
                                                    echo '<td style="width: 100%;">
                                                            ...
                                                            <td>
                                                                <select class="form-control myselect2" name="cbo_pro_code[]" style="width: 100%;">
                                            ...
                                            while ($row_data = mysqli_fetch_object($v_select)) {
                                                echo '<option ' . ($row_child->track_mac_id == $row_data->tr_id ? 'selected' : '') . ' value="' . $row_data->id_a . '" >' . $row_data->name_vn . '</option>';
                                            }
                                            echo '</select>
                                                            </td>
                                                            ...
                                                            <td>
                                                               <select name="cbo_location[]" id="input" class="form-control" style="width: 100%;">
                                                                    <option value="">= Select =</option>';
                                                                        $v_select = $connect->query("SELECT * FROM tbl_st_branch_list 
                                                                            ORDER BY stsup_name DESC");
                                                                        while ($row_data = mysqli_fetch_object($v_select)) {
                                                                            echo '<option '.($row_data->stsup_id==$row_child->locaton_id?'selected':'').' value="'. $row_data->stsup_id .'" >' . $row_data->stsup_name . '</option>';
                                                                        }
                                                                echo '</select>
                                                            </td>
                                                            ...
                                                            <td>
                                                                <input type="number" step="1" name="txt_qty[]" class="form-control" value="'.$row_child->in_qty.'" style="width: 100%;">
                                                            </td>
                                                            <td>
                                                                <select class="form-control myselect2" name="cbo_unit[]" style="width: 100%;">
                                                                    <option value="">= Select =</option>';
                                                            ...
                                                            <td>
                                                                <input type="number" step="0.001" name="txt_price_vn[]" class="form-control" value="'.$row_child->in_price_vn.'" style="width: 100%;">
                                                            </td>
                                                            <td>
                                                                <input type="number" step="0.001" name="txt_price_dollar[]" class="form-control" value="'.$row_child->in_price_dollar.'" style="width: 100%;">
                                                            </td>
                                                            <td>
                                                                <input type="text" name="txt_amo[]" value="'.number_format($row_child->total_amo,2).'" readonly="" id="inputTxt_acc_no" class="form-control" style="width: 100%;">
                                                            </td>
                                                           ';
                                                echo '</tr>';
                                        ...
                                        <tr class="my_form_base" style="background: red; display: none;">
                                            <td>
                                                <input type="text" value="" name="txt_pro_code[]" id="inputTxt_acc_no" class="form-control" readonly="" style="width: 100%;">
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                <select name="cbo_location[]" id="input" class="form-control" style="width: 100%;">
                                                    ...
                                                    while ($row_data = mysqli_fetch_object($v_select)) {
                                                        echo '<option value="' . $row_data->stsup_id . '" >' . $row_data->stsup_name . '</option>';
                                                    }
                                                    ?>
                                                </select>
                                            </td>
                                ...
                                <div class="col-xs-12 col-sm-6 col-md-12 col-lg-12 text-center">
                                    ...
                                    <button type="submit" name="btn_save_in_update" id="btn_save" class="btn blue"><i class="fa fa-save fa-fw"></i>Save</button>
...
<script type="text/javascript">
    ...
    $('#add_more').click(function(){ 
        ...
        $('#myTable').find('tbody tr:eq('+(tr_am-1)+')').after('<tr data-row-id="'+(++index_row)+'">'+$('.my_form_base').html()+'</tr>');
        $('tr[data-row-id='+index_row+']').find('select').select2();
    });
    // setTimeout(function(){
    //     $('#add_more').click();      
    ...
    $('tbody').on('keyup', 'tr td:nth-child(5) >input,tr td:nth-child(7) >input,tr td:nth-child(8) >input', function(event) {
        let v_qty=$(this).parents('tr').find('td:nth-child(5) input').val();
        let v_exchange_rate=$('input[name=txt_exchange_rate]').val();
        let v_price_vn=$(this).parents('tr').find('td:nth-child(7) input').val();
        let v_price_dollar=$(this).parents('tr').find('td:nth-child(8) input').val();
        let v_amo=(v_qty*v_price_vn/v_exchange_rate)+(v_price_dollar*v_qty);
        $(this).parents('tr').find('td:nth-child(9) input').val(v_amo.toFixed(2));
        totalAmount();
    });
    ...
    //         var mc = $('#myTable').find('tbody tr:eq('+i+') td:eq(2) select').val();
    //         var lo = $('#myTable').find('tbody tr:eq('+i+') td:eq(3) select').val();
    //         var qty = $('#myTable').find('tbody tr:eq('+i+') td:eq(4) input').val();
    //         var unit = $('#myTable').find('tbody tr:eq('+i+') td:eq(5) select').val();
    //         var pr_vn = $('#myTable').find('tbody tr:eq('+i+') td:eq(6) input').val();
    //         var pr_d = $('#myTable').find('tbody tr:eq('+i+') td:eq(7) input').val();
    //         if(name==0 || name=='')
    //         {
