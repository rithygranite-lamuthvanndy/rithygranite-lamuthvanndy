                            ...
                            <div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
                                <label>Approved by :
                                    ...
                                    <select class="form-control" name="txt_name" required="required">
                                        ...
                                        <?php 
                                            $v_select = $connect->query("SELECT * FROM tbl_working_check_data ORDER BY wfcd_id ASC");
                                            while ($row_select = mysqli_fetch_object($v_select)) {
                                                if($row_select->wfcd_id==$row_old_data->wfor_check_organizing){
                                                    echo '<option SELECTED="SELECTED" value="'.$row_select->wfcd_id.'">'.$row_select->wfcd_name.'</option>'; 
                                                }else{
                                                    echo '<option value="'.$row_select->wfcd_id.'">'.$row_select->wfcd_name.'</option>';
                                                }
                                            }
                                         ?>
                                    </select>
