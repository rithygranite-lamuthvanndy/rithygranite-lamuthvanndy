...
if(isset($_POST['btn_search'])){
    ...
    $get_data = $connect->query("SELECT A.*,
        ...
        SUM(credit) AS totalCredit
        FROM tbl_acc_rec_adjustment AS A 
        RIGHT JOIN tbl_acc_rec_adjustment_detail AS B ON A.id=B.detail_id
        WHERE DATE_FORMAT(A.date_record,'%Y-%m')='$v_current_year_month'
        GROUP BY A.id
        ORDER BY A.date_record ASC"); 
}
if(isset($_POST['txt_new_row'])){
    $_SESSION['new_row_acc_add_account_adjustment']=$_POST['txt_new_row'];
}
...
<div class="portlet light bordered">
    ...
    <ul class="nav nav-tabs" role="tablist">
        <li role="presentation" class="active">
            <a href="#">Adjustment Record</a>
        </li>
        <li role="presentation">
            <a href="../acc_add_account_stock_inventory/">Stock/ Inventory Record</a>
        </li>
    </ul>
    <br>
    <div class="">
                    ...
                    <li >
                        <a class="tool-action">
                            <i class="icon-printer"></i>
                                Custom Row Page
                                <input name="txt_new_row" form="form" value="<?= (@$_SESSION['new_row_acc_add_account_adjustment'])?(@$_SESSION['new_row_acc_add_account_adjustment']):0 ?>" class="form-control" type="number" onsubmit="this.form,submit()" value="0">
                        </a>
                    </li>
                </ul>
            </div> 
