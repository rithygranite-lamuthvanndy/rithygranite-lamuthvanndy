...
<div class="portlet light bordered">
    ...
    <div class="row">
        <form action="<?= $_SERVER['PHP_SELF'] ?>" method="post">
            ...
            <div class="col-sm-2">
                <div class="" >
                    <select name="txt_employee_name" class="form-control">
                        <option value="">Choose: Employee</option>
                        <?php 
                            $category = $connect->query("SELECT * FROM tbl_employee ORDER BY emp_name ASC");
                            while($row_cate = mysqli_fetch_object($category)){
                                if($row_cate->emp_id == @$_POST['txt_employee_name']){
                                    echo '<option SELECTED value="'.$row_cate->emp_id.'">'.$row_cate->emp_name.'</option>';

                                }else{
                                    echo '<option value="'.$row_cate->emp_id.'">'.$row_cate->emp_name.'</option>';
                ...
                <tbody>
                    <?php
                        ...
                        if(isset($_POST['btn_search'])){
                            ...
                            if(@$_POST['txt_employee_name']!=""){
                                $v_employee = @$_POST['txt_employee_name'];
                                ...
                                FROM  tbl_working_record AS A 
                                    ...
                                    WHERE wr_date BETWEEN '$v_date_s' AND '$v_date_e' AND A.wr_employee='$v_employee'
                                ORDER BY wr_id DESC");
                            }else{
                                $get_data = $connect->query("SELECT 
                                   A.*,U.user_name,Cat.cate_name,Emp.emp_name,C.com_name,D.dep_name
