            ...
            <div class="col-sm-2">
                <div class="" >
                    <select name="txt_category" class="form-control">
                        ...
                        <?php 
                            $category = $connect->query("SELECT * FROM tbl_company ORDER BY com_name ASC");
                            while($row_cate = mysqli_fetch_object($category)){
                                if($row_cate->com_id == @$_POST['txt_category']){
                                    echo '<option SELECTED value="'.$row_cate->com_id.'">'.$row_cate->com_name.'</option>';

                                }else{
                                    echo '<option value="'.$row_cate->com_id.'">'.$row_cate->com_name.'</option>';
                        ...
                        if(isset($_POST['btn_search'])){
                            ...
                            if(@$_POST['txt_category']!=""){
                                ...
                                FROM  tbl_working_record AS A 
                                    ...
                                    WHERE wr_date BETWEEN '$v_date_s' AND '$v_date_e' AND A.wr_company='$v_company'
                                ORDER BY wr_id DESC");
                            }else{
                                $get_data = $connect->query("SELECT 
                                   A.*,U.user_name,Cat.cate_name,Emp.emp_name,C.com_name,D.dep_name
