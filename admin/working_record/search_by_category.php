...
<div class="portlet light bordered">
    <div class="row">
        <div class="col-xs-12">
            <h2><i class="fa fa-fw fa-map-marker"></i> Report: Search by category</h2>
        </div>
    </div>
    <br>
    <div class="row">
            ...
            <div class="col-sm-2">
                <div class="" >
                    <select name="txt_category" class="form-control">
                        ...
                        <?php 
                            $category = $connect->query("SELECT * FROM tbl_ct_category ORDER BY ctcat_name ASC");
                            while($row_cate = mysqli_fetch_object($category)){
                                if($row_cate->ctcat_id == @$_POST['txt_category']){
                                    echo '<option SELECTED value="'.$row_cate->ctcat_id.'">'.$row_cate->ctcat_name.'</option>';

                                }else{
                                    echo '<option value="'.$row_cate->ctcat_id.'">'.$row_cate->ctcat_name.'</option>';
                    ...
                    <?php
                        ...
                        if(isset($_POST['btn_search'])){
                            ...
                            if(@$_POST['txt_category']!=""){
                                ...
                                LEFT JOIN tbl_ct_category AS CC ON CC.ctcat_id=B.ctcom_category
                                WHERE ctco_date_record BETWEEN '$v_date_s' AND '$v_date_e' AND CC.ctcat_id='$v_category'
