...
if (isset($_GET['edit_id'])) {
    $v_id = $_GET['edit_id'];
    $sql = $connect->query("SELECT A.*,cussi_name,
        SUM(slab) AS total_slab,
        SUM(mater) AS total_mater,
        SUM(mater_three) AS total_mater_three,
        ...
        LEFT JOIN tbl_acc_none_sale_revenue_detial AS C ON A.id=C.none_sale_rev_id 
        WHERE A.id='$v_id'");
...
//     $sql=$connect->query("SELECT id FROM tbl_acc_add_tran_amount WHERE ref_id='$v_id' AND status_type='2'");
//     $row=mysqli_fetch_object($sql);
//     $continue_update_id=@$row->id;
//     // echo $continue_update_id;
//     if($continue_update_id)
//         header( "refresh:5; url=../acc_add_tra_amount/edit.php?edit_id=".$continue_update_id.'&status=con_update_sale_revenue'); 
// }
if (isset($_POST['btn_save'])) {
    ...
    $query_update = "UPDATE tbl_acc_none_sale_revenue SET 
        cus_id='$v_cus_name',
        inv_no='$v_inv_no', 
        date_record='$v_date_record', 
        po_no='$v_po_no', 
        delivery_no='$v_deli_no', 
        delivery_date='$v_del_date', 
    ...
    foreach ($v_pro_name as $key => $value) {
        if ($value != 0 && $v_status[$key] == 0) {
            ...
            $new_amo = $v_amo[$key];
            //Add Sub Item
            $query_add_2 = "INSERT INTO tbl_acc_none_sale_revenue_detial(
                    none_sale_rev_id, 
                    code, 
                    inv_pro_id, 
                    fea_id, 
            ...
            $new_unit_price = $v_unit_price[$key];
            $new_amo = $v_amo[$key];
            $query_update = "UPDATE tbl_acc_none_sale_revenue_detial SET 
                    none_sale_rev_id='$v_main_id', 
                    code='$new_code', 
                    inv_pro_id='$new_pro_name', 
                    fea_id='$new_fea', 
                    length='$new_length', 
                    width='$new_width', 
                        ...
                        <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="cbo_cus_name" id="input" class="form-control myselect2" required="required">
                                        ...
                                        while ($row_select = mysqli_fetch_object($sql)) {
                                            if ($row_main->cus_id == $row_select->cussi_id)
                                                echo '<option selected value="' . $row_select->cussi_id . '">' . $row_select->cussi_name . '</option>';
                                            else
                            ...
                            <div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="txt_inv_no" class="form-control" required="" value="<?= $row_main->inv_no ?>" placeholder="Invoice No .....">
                                </div>
                                <div class="form-group">
                                    <label>Date : </label>
                                    <input type="text" name="txt_date_record" data-provide="datepicker" data-date-format="yyyy-mm-dd" class="form-control" required value="<?= $row_main->date_record ?>">
                                </div>
                                <div class="form-group">
                                    <label>PO No : </label>
                                    <input type="text" class="form-control" required name="txt_po_no" value="<?= $row_main->po_no ?>">
                                </div>
                                <div class="form-group">
                                    <label>Delivery No : </label>
                                    <input type="text" class="form-control" required name="txt_deli_no" value="<?= $row_main->delivery_no ?>" placeholder="Delivery No ....">
                                </div>
                                <div class="form-group">
                                    <label>Delivery Date : </label>
                                    <input type="text" name="txt_del_date" data-provide="datepicker" value="<?= $row_main->delivery_date ?>" data-date-format="yyyy-mm-dd" class="form-control" required value="<?= date('Y-m-d') ?>">
                                </div>
                            </div>
                            ...
                            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                                ...
                                <table id="myTable" class="table table-bordered table-responsive">
                                    ...
                                    <tbody>
                                        <?php
                                        $sql = $connect->query("SELECT * FROM tbl_acc_none_sale_revenue_detial WHERE none_sale_rev_id='$row_main->id'");
                                        while ($row_detail = mysqli_fetch_object($sql)) {
                                            echo '<tr>';
                                            echo '<td>
                                            <input name="txt_status[]" value="' . $row_detail->id . '" type="hidden">
                                                                <select class="form-control myselect2" onchange="changeInvCode(this);" name="cbo_code[]">
                                            ...
                                            while ($row_data_1 = mysqli_fetch_object($v_select)) {
                                                if ($row_detail->inv_pro_id == $row_data_1->inv_pron_id)
                                                    echo '<option selected data_id="' . $row_data_1->inv_pron_id . '" value="' . $row_data_1->inv_pron_code . '">' . $row_data_1->inv_pron_code . '</option>';
                                                else
                                                    echo '<option data_id="' . $row_data_1->inv_pron_id . '" value="' . $row_data_1->inv_pron_code . '">' . $row_data_1->inv_pron_code . '</option>';
                                            }
                                            '</select>
                                                            </td>';
                                            echo '<td>';
                                            echo '<select class="form-control myselect2" onchange="changeInvName(this);" name="cbo_pro_name[]">';
                                            echo '<option value="">=== Please Choose and Select ===</option>';
                                            $v_select = $connect->query("SELECT inv_pron_id,inv_pron_name_en,inv_pron_code FROM tbl_inv_product_name ORDER BY inv_pron_name_en DESC");
                                            while ($row_data = mysqli_fetch_object($v_select)) {
                                                if ($row_detail->inv_pro_id == $row_data->inv_pron_id)
                                            ...
                                            echo '</select>
                                                        ...
                                                        <td>
                                                            <select class="form-control myselect2" name="cbo_fea[]">
                                                                <option value="">=== Please Choose and Select ===</option>';
                                            $v_select = $connect->query("SELECT id,name FROM tbl_inv_feature ORDER BY name DESC");
                                            while ($row_data = mysqli_fetch_object($v_select)) {
                                                if ($row_detail->fea_id == $row_data->id)
                                            ...
                                            echo '</select>
                                                        ...
                                                        <td>
                                                            <input type="text" name="txt_length[]" class="form-control" value="' . $row_detail->length . '">
                                                        </td>
                                                        <td>
                                                            <input type="text" name="txt_width[]" class="form-control" value="' . $row_detail->width . '">
                                                        </td>
                                                        <td>
                                                            <input type="text" name="txt_thickness[]" class="form-control" value="' . $row_detail->thickness . '">
                                                        </td>
                                                        <td>
                                                            <input type="text" onkeyup="getSlab(this)" onchange="getSlab(this)" name="txt_slab[]" class="form-control" value="' . $row_detail->slab . '">
                                                        </td>    
                                                        <td>
                                                            <input type="text" onkeyup="getMater(this)" onchange="getMater(this)" name="txt_mater[]" class="form-control" value="' . $row_detail->mater . '">
                                                        </td>
                                                         <td>
                                                            <input type="text" onkeyup="getMater_three(this)" onchange="getMater_three(this)" name="txt_mater_three[]" class="form-control" value="' . $row_detail->mater_three . '">
                                                        </td>
                                                        <td>
                                                            <input type="text" onkeyup="getAmount(this);" name="txt_unit_price[]" onchange="getAmount(this);" class="form-control" value="' . $row_detail->unit_price . '">
                                                            </td>
                                                        <td>
                                                            <input type="text" name="txt_amo[]" readonly="" id="inputTxt_acc_no" class="form-control" value="' . $row_detail->amount . '">
                                                        </td>
                                        ...
                                        <tr class="my_form_base" style="background: red; display: none;">
                                            ...
                                            <td>
                                                <select class="form-control" name="cbo_pro_name[]">
                                                    <option value="">=== Please Choose and Select ===</option>
                                                    <?php
                                                    $v_select = $connect->query("SELECT inv_pron_id,inv_pron_name_en,inv_pron_code FROM tbl_inv_product_name ORDER BY inv_pron_name_en DESC");
                                                    while ($row_data = mysqli_fetch_object($v_select)) {
                                    ...
                                    <tfoot>
                                        <tr>
                                            ...
                                            <th><input type="text" name="txt_total_slab" readonly="" id="inputTxt_total_amo" class="form-control" value="<?= $row_main->total_slab ?>" required="required"></th>
                                            <th><input type="text" name="txt_total_mater" readonly="" id="inputTxt_total_amo" class="form-control" value="<?= number_format($row_main->total_mater, 2) ?>" required="required"></th>
                                             <th><input type="text" name="txt_total_mater_three" readonly="" id="inputTxt_total_amo_three" class="form-control" value="<?= number_format($row_main->total_mater_three, 2) ?>" required="required"></th>
                                            <th></th>
                                            <th><input type="text" name="txt_total_amount" readonly="" id="inputTxt_total_amo" class="form-control" value="<?= number_format($row_main->total_amo, 2) ?>" required="required"></th>
                                        </tr>
                                        <tr>
                                            ...
                                            <th><input type="number" step="0.01" min="0" name="txt_total_dis_amount" class="form-control" value="<?= $row_main->total_discount ?>"></th>
                                        </tr>
                                        <tr>
                                            <th style="visibility: hidden;"></th>
                                            <th style="visibility: hidden;"></th>
                                            ...
                                            <th><input type="text" name="txt_total_grand_total" class="form-control" value="<?= number_format($row_main->grand_toal, 2) ?>" readonly=""></th>
                                        </tr>
                                    </tfoot>
                                </table>
                                <div class="row">
...
<script type="text/javascript">
    ...
    $("tbody").on('click', '.btnDelete', function() {
        ...
        var sub_id = $(this).parents('tbody >tr').find('td:nth-child(1) >input').val();
        var ans = confirm("Are you sure to delete this record ?");
        if (ans) {
            $.ajax({
                url: 'ajax_delete.php?del_id=' + sub_id,
                async: false,
    ...
    function changeInvCode(args) {
        ...
        $.ajax({
            url: 'ajx_get_content_select.php?p_inv_code=' + v_inv_id,
            success: function(result) {
    ...
    function changeInvName(args) {
        ...
        $.ajax({
            url: 'ajx_get_content_select.php?p_inv_name=' + v_inv_id,
            success: function(result) {
    ...
    var v_dis = $('tfoot >tr:nth-last-child(2) >th >input').val();
    let v_total_amo = totalAmount();
    $('tfoot >tr:last-child >th >input').val((v_total_amo - v_dis));

    $('tfoot >tr:nth-last-child(2) >th >input').on('keyup', function() {
        let v_dis = parseFloat($(this).val());
        let v_total_amo = totalAmount();
        $('tfoot >tr:last-child >th >input').val((v_total_amo - v_dis));
    });
    $(document).ready(function() {
        let v_cus_id = $('select[name=cbo_cus_name]').val();
        $.ajax({
            url: 'ajx_get_cus_info.php?v_cus_id=' + v_cus_id,
            success: function(result) {
        ...
        $('#re_inv_name').click(function() {
            $.ajax({
                url: 'ajx_get_content_select.php?status=cbo_pro_name',
                success: function(result) {
        ...
        $('#re_fea_name').click(function() {
            $.ajax({
                url: 'ajx_get_content_select.php?status=cbo_fea',
                success: function(result) {
