...
<?php
    if(@$_GET['parent']){
        $v_id=@$_GET['parent'];
        $sql="SELECT *,(out_price*out_qty) AS amo,B.stpron_code,B.stpron_name_kh,user_name
...
<div class="portlet light bordered">
    <div class="row">
        <div class="text-center text-primary">
            <h2 class="bold text-uppercase">Approved Stock Out</h2>
        </div>
    </div>
    <hr>
                    ...
                    <?php
                        ...
                        while ($row = mysqli_fetch_object($result_detail)) {      
                            echo '<tr>';
                                ...
                                echo '<td class="text-left">'.$row->out_qty.'</td>';
                                echo '<td class="text-left">'.number_format($row->out_price,2).'</td>';
                                echo '<td class="text-left">'.number_format($row->amo,2).'</td>';
                                echo '<td class="text-left">'.$row->user_name.'</td>';
                                echo '<td class="text-left">';
                                        echo '<label class="switch">';
                                                echo '<input style="display: none;" type="checkbox" id="togBtn" data-parent-id='.$row->std_id.' name="rdoAppr" onchange="change_view(this);" '.(($row->out_approved)?('checked'):('')).'>
                                                        <div class="slider round">
                                                            <span class="on">YES</span>
                                                            <span class="off">NO</span>
                                                        </div>';
