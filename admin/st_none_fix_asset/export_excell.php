...
$v_sql = "SELECT A.*,B.stca_name,B.stca_code,C.stun_name,B.stca_id,
                ...
                (
                    ...
                    WHERE  AA.stsadj_date_record BETWEEN '{$v_date_start}' AND '{$v_date_end}'
                    AND BB.pro_id=A.stpron_id
                )<>0 ORDER BY B.stca_name ASC,A.stpron_code ASC
                ";
// exit($v_sql);
...
$objPHPExcel->getActiveSheet()->setCellValue('E' . 1, 'តារាងស្ដុកឧបករណ៍ជាង នឹង ម៉ាស៊ីនខា្នតតូច'.' '.date('d/m/Y', strtotime($v_date_start)).'-'. date('d/m/Y', strtotime($v_date_end)));
$objPHPExcel->getActiveSheet()->setCellValue('E' . 2, 'Bảng Kho Thiết Bị Và Máy Kích Thước Nhỏ'.' '.date('d/m/Y', strtotime($v_date_start)).'-'. date('d/m/Y', strtotime($v_date_end)));
// Header
...
$in_plus=$header_bootom+1;
  $v_sqls = "SELECT * FROM tbl_st_category_list WHERE material_type_id='$_SESSION[status]' ORDER BY stca_code";
    $v_result_summary = $connect->query($v_sqls);
