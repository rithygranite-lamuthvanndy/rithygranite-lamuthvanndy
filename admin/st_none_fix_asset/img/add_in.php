    ...
    <div class="portlet-body">
        <div class="panel panel-primary">
            ...
            <div class="panel-body">
                <div class="col-lg-12">
                    <button class="tablink" id="btn_reg" style="background-color: red;">Register</button>
                    <button class="tablink" id="btn_assign">Assigning</button>
                    <button class="tablink" id="btn_write">Written Off</button>
                    <!-- <button class="tablink" onclick="openPage('Contact', this, 'blue')">Data Entry Page 3</button> -->
                    <button class="tablink" id="btn_view">View</button>
                </div>
                            ...
                            <div class="row">
                                <div class="col-lg-6" id="1cat_container">
                                    <div class="form-group row">
                                            ...
                                            <label for="colFormLabel" class="col-sm-2 col-form-label" style="color: black;">Category</label>
                                                <div class="col-sm-8">
                                                    <select name="asset_cat" id="asset_cat" class="form-control">
                                                        <?php
                                                            $stml_select = "SELECT * FROM tbl_non_cat";
                                ...
                                <div class="col-lg-6">
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-6" id="cat_container">
                                            <input type="text" name="asset_id" class="form-control" value="" id="asset_id" placeholder="" readonly="">
...
<script type="text/javascript">
    $(document).ready(function()
    {
        ...
        body.on('click','#btn_add',  function()
        {
            ...
            {
                assign_to = $('#assign_to option:selected').text();
                assign_to_val = $('#assign_to option:selected').val();
                assign_date = $('#assign_date').val();

                body.find('#form2list').html(lst2);
            }
        ...
        // function save
        function save(frm_data, wis)  // wis = which is button
        {
            $.ajax({
                url:"save/savedata.php",
                type:"POST",
                data: frm_data,
                cache:false,
                processData:false,
                ...
                success: function(data)
                {
                    ...
                    if(data.msg=="success")
                    {
                        ...
                        if(wis==1)
                        {
                            $('#form1').html(form1); // put entry 1 (no data like as add new)
                            body.find('#last_id').val(data.last_id); // set last_id to Register
                        ...
                        {
                            // var list = $('#form2list').html();
                            var cdt = body.find('#cdt').val();
                            // append data to tr
                            // $('#form2list').html(list);
                            var tr = body.find('#form2list tr').length;
                            assign_to_val = assign_to_val.substring(2);
                            var data_tr = "<tr><td>"+assign_date+"<input type='hidden' value="+data.last_id+"></td><td>"+cdt+"</td><td>"+assign_to+"</td><td>"+assign_to_val+"</td></tr>";
                            
                            var isedit = body.find('#isedit').val();
                            if(isedit==0)
                            {
                                $('#form2list').find('tr:eq(0)').after(data_tr);
                            }
                            else
                            {
                                $('#form2list').find('tr:eq('+tr_ind+') td:eq(0)').text(assign_date);
                                $('#form2list').find('tr:eq('+tr_ind+') td:eq(0)').append('<input type="hidden" value='+id+'>');
                                $('#form2list').find('tr:eq('+tr_ind+') td:eq(2)').text(assign_to);
                                body.find('#isedit').val(0);
                            }
                            body.find('#ass_id').val(data.last_id);
                            ass_last_id = data.last_id;
                            lst2 = body.find('#form2list').html(); // get list from Assigning page before form3 put
                        }
                        else if(wis==3)
                        {
                            ...
                            $('#form2list').html(list);
                            var tr = body.find('#form2list tr').length;
                            var data_tr = "<tr><td>test<input type='hidden' value="+data.last_id+"></td><td>"+cdt+"</td><td>"+adj_text+"</td><td>"+adj_val+"</td></tr>";
                            ...
                            {
                                $('#form2list').find('tr:eq('+tr_ind+') td:eq(0)').text('test');
                                $('#form2list').find('tr:eq('+tr_ind+') td:eq(0)').append('<input type="hidden" value='+id+'>');
                                $('#form2list').find('tr:eq('+tr_ind+') td:eq(1)').text(cdt);
                                $('#form2list').find('tr:eq('+tr_ind+') td:eq(2)').text(adj_text);
                                $('#form2list').find('tr:eq('+tr_ind+') td:eq(3)').text(adj_val);
                                body.find('#isedit').val(0);
                            }
                            

                            lst3 = body.find('#form2list').html(); // get list from Assigning page before form4 put   
                        }
                    }
                    else if(data.msg=='update success')
                    {
                        alert(data.msg);
                        assign_to = body.find('#assign_to option:selected').text();
                        assign_date = body.find('#assign_date').val();
                        
                        $('#form2list').find('tr:eq('+tr_ind+') td:eq(0)').html(assign_date);
                        $('#form2list').find('tr:eq('+tr_ind+') td:eq(2)').html(assign_to);
                        body.find('#isedit').val(0);
                    }
                }
            });
