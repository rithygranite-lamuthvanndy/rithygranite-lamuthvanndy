...
if (isset($_GET['p_date_start']) || isset($_GET['p_date_end'])) {
 ...
 if (isset($_GET['p_date_start']) && isset($_GET['p_date_end'])) {
    ...
    $sql = "SELECT
        ...
        F.name_vn AS track_machine,G.stsup_name,
        (
            CASE C.locaton_id
                WHEN 0 THEN 'រោងចក្រ'
                WHEN 1 THEN 'រណ្ដៅ'
        ...
        LEFT JOIN tbl_st_branch_list AS G ON G.stsup_id=C.locaton_id
...
$objPHPExcel->getActiveSheet()->setCellValue('L' . 5, 'Máy móc / máy móc');
  ...
  $i=1;
   ...
   while ($row = mysqli_fetch_object($get_data)) {
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('H' . $idx, $row->stsup_name);
